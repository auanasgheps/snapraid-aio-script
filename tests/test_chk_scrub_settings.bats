...
@test "chk_scrub_settings: missing counter file treated as 0 — increments to 1, no scrub" {
  SCRUB_DELAYED_RUN=3
  rm -f "$SCRUB_COUNT_FILE"
  chk_scrub_settings || true
  [ "$RUN_SCRUB_CALLED" -eq 0 ]
  [ "$(cat "$SCRUB_COUNT_FILE")" -eq 1 ]
}
...
@test "chk_scrub_settings: counter at threshold — runs scrub and resets counter file" {
  SCRUB_DELAYED_RUN=3
